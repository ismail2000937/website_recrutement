...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM recruteur WHERE email = ?');
    $stmt->execute([$email]);
    $rec = $stmt->fetch();

  // Verify password 
  if ($rec && ($password===$rec['password'])) {
      $_SESSION['id_recruteur'] = $rec['id_recruteur'];
...
<body>
    ...
    <div >
        <em class="con">Connexion Recructeur</em>
    </div>
    <fieldset>
    ...
    <form class="main" method="POST" action="">
      <input type="email" name="email" placeholder="entrer votre email"> </div></br>
       <input type="password" name="password"  id="pass"  placeholder="entrer votre password"> </br>
        <input type="submit" value="connexion" class="connexion"> </br></br>
        <p>N'avez pas de compte?<a href="recruteur.php" >S'inscrire</a></p>
    </form>   
    </fieldset>
